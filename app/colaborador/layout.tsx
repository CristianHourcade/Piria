...
export default async function ColaboradorLayout({
  ...
  if (SHOULD_LOG) {
    console.log("🔐 [Server] Datos de usuario en Colaborador Layout:")
  ...
  // Both admin and colaborador roles can access colaborador routes
  if (userRole !== "admin" && userRole !== "colaborador") {
    console.log("🚫 [Server] Usuario no tiene rol válido, redirigiendo a login")
    redirect("/login")
  }

  if (SHOULD_LOG) {
    console.log("✅ [Server] Usuario con acceso a colaborador confirmado, mostrando layout")
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <Sidebar role={userRole === "admin" ? "admin" : "colaborador"} />
